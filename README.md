...
| `popup.js` | 690 | Popup 交互、过滤、下载、AI 配置、四角拖拽缩放 |
| `popup.css` | 527 | UI 样式与四角拖拽缩放样式 |
| `popup.html` | 158 | 页面结构与内联尺寸兜底 |
...
| 版本 | 说明 |
|------|------|
| v0.0.2 | 新增单条请求 AI 分析、独立模型列表 URL、模型下拉选择、AI 结果区滚动优化、Popup 四角等比例拖拽缩放 |
...
- 默认 Popup 通过内联关键样式和实体容器双重兜底强制撑开为 800x600，鼠标悬停四个对角可拖拽等比例缩放；过小、异常或旧版历史缩放会自动重置，缩放状态自动记忆
